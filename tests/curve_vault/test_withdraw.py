...
def test_vault_full_withdraw_single_user(
    ...
    with boa.env.prank(user):
        pyusd_crvusd_pool.approve(vault_addr, user_lp_balance)
...
def test_vault_withdraw_for_another_user(
    ...
    owner_lp_balance = pyusd_crvusd_pool.balanceOf(owner)
    ...
    withdraw_assets = vault_contract.convertToAssets(shares_received // 2)
    initial_receiver_lp_balance = pyusd_crvusd_pool.balanceOf(receiver)
    ...
    final_receiver_lp_balance = pyusd_crvusd_pool.balanceOf(receiver)
...
def test_vault_withdraw_more_than_balance_reverts(
    test_permissioned_vault, funded_accounts, pyusd_crvusd_pool
):
    vault_addr, strategy_addr, harvester_addr = test_permissioned_vault
