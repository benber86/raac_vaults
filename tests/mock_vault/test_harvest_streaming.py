...
def test_harvest_streaming_rewards(
    ...
    print("Initial state:")
    print(f"Total deposited: {total_deposited / 10**18:.1f}")
    print(f"Total shares: {total_shares_before / 10**18:.1f}")
    print(f"Total assets: {mock_vault.totalAssets() / 10**18:.1f}")

    harvest_amount = int(50_000 * 10**18)

    expected_locked_shares = mock_vault.convertToShares(harvest_amount)
    print(f"Expected locked shares: {expected_locked_shares / 10**18:.1f}")
    ...
    print(f"User1 max withdraw: {user1_max_withdraw_start / 10**18:.1f}")
    print(f"User2 max withdraw: {user2_max_withdraw_start / 10**18:.1f}")
    ...
    print(f"User1 max withdraw: {user1_max_withdraw_half / 10**18:.1f}")
    print(f"User2 max withdraw: {user2_max_withdraw_half / 10**18:.1f}")
    print(f"Unlocked shares: {mock_vault.unlocked_shares() / 10**18:.1f}")
    print(
        f"Locked shares: {mock_vault.balanceOf(mock_vault.address) / 10**18:.1f}"
    )
    ...
    print(f"User1 max withdraw: {user1_max_withdraw_end / 10**18:.1f}")
    print(f"User2 max withdraw: {user2_max_withdraw_end / 10**18:.1f}")
    print(f"Unlocked shares: {mock_vault.unlocked_shares() / 10**18:.1f}")
    print(
        f"Locked shares: {mock_vault.balanceOf(mock_vault.address) / 10**18:.1f}"
    )
