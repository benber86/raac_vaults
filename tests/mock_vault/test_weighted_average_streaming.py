...
def test_weighted_average_profit_locking(
    ...
    print(f"\nAfter first harvest ({first_harvest / 10**18:.1f}):")
    print(f"Expected locked shares: {expected_locked_shares_1 / 10**18:.1f}")
    print(
        f"Actual locked shares: {mock_vault.balanceOf(mock_vault.address) / 10**18:.1f}"
    )
    print(f"Unlocked shares: {mock_vault.unlocked_shares() / 10**18:.1f}")
    ...
    print(f"Unlocked shares: {mock_vault.unlocked_shares() / 10**18:.1f}")
    print(
        f"Locked shares: {mock_vault.balanceOf(mock_vault.address) / 10**18:.1f}"
    )

    second_harvest = int(30_000 * 10**18)
    ...
    print(f"\nBefore second harvest ({second_harvest / 10**18:.1f}):")
    print(f"Existing locked shares: {existing_locked_shares / 10**18:.1f}")
    print(
        f"Expected new locked shares: {expected_locked_shares_2 / 10**18:.1f}"
    )
    ...
    print(f"Expected total locked shares: {total_locked_shares / 10**18:.1f}")
    print(f"Actual total locked shares: {actual_locked_shares / 10**18:.1f}")
    ...
    print(
        f"Expected unlocked at half: {expected_unlocked_at_half / 10**18:.1f}"
    )
    print(f"Actual unlocked at half: {actual_unlocked_at_half / 10**18:.1f}")
    ...
    print(f"User1 withdrawable: {user_withdrawable / 10**18:.2f}")
    print(f"Expected withdrawable: {expected_withdrawable / 10**18:.2f}")
    ...
    print(f"Total expected profit: {total_expected_profit / 10**18:.1f}")
    print(f"Expected final withdrawable: {total_expected_final / 10**18:.1f}")
    print(f"Actual final withdrawable: {final_withdrawable / 10**18:.1f}")
